...
public class PathfindingController {
    ...
    /**
     * Checks if a specified point on the grid is attackable.
     *
     * @param grid  The 2D grid representing the environment.
     * @param point The point to be checked for attackability.
     * @return      True if the point is within the grid boundaries and is not occupied; false otherwise.
     */
    public static boolean IsAttackable(Element[][] grid, Point point) {
        // Check if the point is within the grid boundaries
        if (point.x < 0 || point.x > grid.length - 1) return false;
        if (point.y < 0 || point.y > grid[0].length - 1) return false;
        ;
        // Check if the point is not occupied
        return grid[point.x][point.y] == null || grid[point.x][point.y].getClass() != Obstacle.class;
    }
     ...
     */
    public static List<Point> FindNeighbors(Element[][] grid, Point point, boolean pathAttack) {
        ...
        if(pathAttack){
            if (IsAttackable(grid, up)) neighbors.add(up);
            if (IsAttackable(grid, down)) neighbors.add(down);
            if (IsAttackable(grid, left)) neighbors.add(left);
            if (IsAttackable(grid, right)) neighbors.add(right);
        } else {
     ...
     */
    public static int[][] FindAllPaths(Element[][] grid, Point start, int maxDistance, boolean pathAttack) {
        ...
        FindPathsRecursively(grid, start, maxDistance, currentPath, allPointsList, pathAttack);
     ...
     */
    private static void FindPathsRecursively(Element[][] grid, Point current, int maxDistance, List<Point> currentPath, List<int[]> allPointsList, boolean pathAttack) {
        ...
        for (Point neighbor : FindNeighbors(grid, current, pathAttack)) {
            ...
            if (!currentPath.contains(neighbor)) {
                ...
                FindPathsRecursively(grid, neighbor, maxDistance - 1, currentPath, allPointsList, pathAttack);
     ...
     */
    public static List<Point> FindPathMonster(Element[][] grid, Point start, Point end, int proximityThreshold, boolean pathAttack) {
        ...
        while (!finished) {
            ...
            for (int i = 0; i < used.size(); ++i) {
                ...
                for (Point neighbor : FindNeighbors(grid, point, pathAttack)) {
